          ...
          then
            ...
            ( (abi, added_func),
              ( func_name,
                ...
                (match ms with Some _ -> true | None -> false),
                return_func,
                List.length outputs ) )
          else raise Not_implemented)
  | _ -> raise Not_allowed_function
  ...
  | { vb_pat = { pat_desc = Tpat_var (func_name, _); _ }; vb_expr = e; _ } :: []
    -> (
      ...
      ( translate_function func_uniq_name e,
        ...
        match
          List.find_opt
            (fun (x, _, _, _, _, _) -> x = Ident.name func_name)
            func_sigs
        with
        | Some (_, sig_string, params, is_mut_arg, returner, return_num) ->
      ...
      let rec backend_aux = function
        ...
        | [
                          ...
                          Tmod_constraint
                            ( {
                              ...
                              Tmodtype_explicit
                                {
                                  mty_desc =
                                    Tmty_signature { sig_items = sigs; _ };
                                  _;
                                },
                              _ );
                        _;
            ...
            let funcs, cases =
              List.split
                (List.map
                   (translate_external_func func_sigs storage_num
                      mut_storage_num)
                   exps)
            in
            ...
            let default_function_defs =
              ...
              in
              storage_operations @ mut_storage_operations
            ...
            (* print debugging *)
            (* if true then (
                 print_endline (Yojson.Basic.pretty_to_string (json_of_abis abis));
